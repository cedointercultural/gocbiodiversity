...
#' @export
execute_bio_queries <- function(config_file,
                                         polygon_file = NULL,
                                         output_dir = NULL) {
  ...
  # ========================================================================
  # PASO 3: Generar cuadrilla de poligonos para búsqueda
  # ========================================================================
  ...
  # ========================================================================
  # PASO 4: Generar cuadrilla de puntos para busqueda
  # ========================================================================

  if (config$spatial$grid_enabled) {
    point_grid <- generate_grid_points(
      polygon = polygon,
      grid_size = config$spatial$grid_size_degrees,
      buffer_percent = if(!is.null(config$spatial$buffer_percent)) config$spatial$buffer_percent else 2
    )
  ...
  # ========================================================================
  # PASO 5: Ejecutar consultas a bases de datos
  # ========================================================================
  log_fn("───────────────────────────────────────────────────────────────", log_file, level="INFO")
  ...
  # Decidir estrategia de ejecución (secuencial vs paralela)
    ...
    execute_queries_loop(
      ...
      log_function,
      log_file,
      this_source= "fishbase"
    )
    # execute_obis_queries_parallel(
    #   grid=grid,
    #   config=config,
    #   log_function,
    #   log_file,
