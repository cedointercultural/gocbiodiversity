#' @title Ejecutar consultas en paralelo para GBIF
...
#' @export
execute_gbif_queries_parallel <- function(grid,
                                          config,
                                          log_function,
                                          log_file,
                                          n_cores = NULL) {
  ...
  if (config$databases$gbif$enabled) {
    ...
    parallel::clusterExport(cl, c("query_gbif", "get_spatial_format_for_api"))
