function (names, data_sources = c(1, 12), all_matches = FALSE,
          capitalize = FALSE, species_group = FALSE, fuzzy_uninomial = FALSE,
          stats = FALSE, main_taxon_threshold = 0.5, output_type = "table",
          ...)
{
  batch_size <- 100
  ...
  if (length(main_taxon_threshold) != 1 || !is.numeric(main_taxon_threshold) ||
      main_taxon_threshold < 0.5 || main_taxon_threshold >
      1) {
    stop(call. = FALSE, "The `main_taxon_threshold` input must be a single number between 0.5 to 1")
  }
  output_type <- match.arg(output_type, c("table", "list",
                                          "json"))
  ...
  batch_data <- lapply(name_batches, function(batch) {
    ...
    args <- c(data_sources = paste0(data_sources, collapse = "|"),
              all_matches = tolower(as.character(all_matches)),
              capitalize = tolower(as.character(capitalize)), species_group = tolower(as.character(species_group)),
              fuzzy_uninomial = tolower(as.character(fuzzy_uninomial)),
              stats = tolower(as.character(stats)))
    formatted_args <- paste0(paste0(names(args), "=", args),
                             collapse = "&")
    formatted_path <- paste0("api/v1/verifications/", paste0(batch,
                                                             collapse = "|"), "?", formatted_args)
  ...
  used_cols <- c("submittedName", "dataSourceId", "dataSourceTitleShort",
                 "curation", "recordId", "entryDate", "sortScore", "matchedNameID",
                 "matchedName", "matchedCardinality", "matchedCanonicalSimple",
                 "matchedCanonicalFull", "currentRecordId", "currentNameId",
                 "currentName", "currentCardinality", "currentCanonicalSimple",
                 "currentCanonicalFull", "taxonomicStatus", "isSynonym",
                 "editDistance", "stemEditDistance", "matchType", "cardinalityScore",
                 "infraSpecificRankScore", "fuzzyLessScore", "curatedDataScore",
                 "authorMatchScore", "acceptedNameScore", "parsingQualityScore")
  ...
  if (all_matches) {
    response_table <- do.call(rbind, lapply(response_data,
                                            function(x) {
                                              do.call(rbind, lapply(x$results, function(y) convert_entry_to_row(y,
                                                                                                                x$name)))
                                            }))
  }
  else {
  ...
  submitted_names <- vapply(split_table, function(x) x$submittedName[1],
                            FUN.VALUE = character(1))
  response_table <- do.call(rbind, split_table[match(names,
                                                     submitted_names)])
  row.names(response_table) <- NULL
  numeric_cols <- c("sortScore", "matchedCardinality", "currentCardinality",
                    "editDistance", "stemEditDistance", "cardinalityScore",
                    "infraSpecificRankScore", "fuzzyLessScore", "curatedDataScore",
                    "authorMatchScore", "acceptedNameScore", "parsingQualityScore")
  response_table[numeric_cols] <- lapply(response_table[numeric_cols],
                                         as.numeric)
